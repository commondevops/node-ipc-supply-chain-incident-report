 ...
 * document (inlined CSS, no external assets), with internal .md links
...
const REPORT_TITLE = 'node-ipc Supply Chain Incident Report';
...
function parseBlocks(lines, headings) {
  ...
  while (i < n) {
    ...
    if (headingMatch) {
      ...
      if (!(level === 1 && plainText === REPORT_TITLE)) {
...
function buildDocument(sections, fileCount) {
  ...
  return (
    ...
    `</head>\n` +
    `<body id="top">\n` +
    `<header class="report-header"><h1>${REPORT_TITLE}</h1></header>\n` +
    `<main class="content">\n${content}\n</main>\n` +
