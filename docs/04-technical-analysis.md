...
The malicious code was appended to `node-ipc.cjs`, the CommonJS entry point, while the ESM entry point `node-ipc.js` and the other source files were clean (documented in Section 2.1). This distinction matters operationally: package presence alone was not equivalent to malicious execution, and the highest-risk condition was a consumer environment that resolved an affected version and then loaded the compromised CommonJS path. The full exposure sequence and the capability-vs-impact boundary are assessed in Section 3.3.
